...
} from "@/app/production-orders/components/productionOrderUi";

const EMPTY_VALUES = Object.fromEntries(PRODUCTION_ORDER_FILTER_KEYS.map((k) => [k, ""]));

export { PRODUCTION_ORDER_FILTER_KEYS };

export default function ProductionOrderFilterPanel({
  open: controlledOpen,
  onToggle,
  values = {},
  onApply,
  ...
  const getAppliedFiltersSummary = () => {
    const labels = {
      ...
      status: "Status",
      priority: "Priority",
      planned_start_date_from: "Start From",
      planned_start_date_to: "Start To",
      planned_end_date_from: "End From",
      planned_end_date_to: "End To",
    ...
    return Object.entries(values || {})
      .filter(
        ([key, v]) =>
          PRODUCTION_ORDER_FILTER_KEYS.includes(key) &&
          key !== "planned_start_date_op" &&
          v != null &&
          String(v).trim() !== ""
      )
    ...
    <Card className="rounded-xl shadow-sm border-slate-200 bg-white mb-2 overflow-visible">
      ...
      {open && (
        <div className="border-t border-slate-100 px-2.5 py-2 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-2 bg-slate-50/30">
          ...
          >
            <MenuItem value="">All</MenuItem>
            {PRODUCTION_ORDER_PRIORITY_OPTIONS.map((o) => (
              <MenuItem key={o.value} value={o.value}>
                {o.label}
              </MenuItem>
            ))}
          ...
          <Checkbox
            name="open_only"
            label={`Open ${AP.orders.title.toLowerCase()} only`}
            checked={localValues.open_only === true || localValues.open_only === "true"}
            onCheckedChange={(checked) => handleChange("open_only", checked ? "true" : "")}
          />

          <DateField
            name="planned_start_date_from"
            label="Planned Start From"
            value={localValues.planned_start_date_from}
            onChange={(e) => handleChange("planned_start_date_from", e.target.value)}
          />
          <DateField
            name="planned_start_date_to"
            label="Planned Start To"
            value={localValues.planned_start_date_to}
            onChange={(e) => handleChange("planned_start_date_to", e.target.value)}
          />
          <DateField
            name="planned_end_date_from"
            label="Planned End From"
            value={localValues.planned_end_date_from}
            onChange={(e) => handleChange("planned_end_date_from", e.target.value)}
          />
          <DateField
            name="planned_end_date_to"
            label="Planned End To"
            value={localValues.planned_end_date_to}
            onChange={(e) => handleChange("planned_end_date_to", e.target.value)}
          />
          <DateField

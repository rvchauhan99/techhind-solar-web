...
function flattenB2bRow(row) {
  if (!row) return row;
  const so = row.salesOrder || row.b2bSalesOrder || null;
  return {
    ...row,
    b2b_sales_order_id: row.b2b_sales_order_id || so?.id || null,
    order_no: so?.order_no || row.order_no || null,
    client_name: so?.client?.client_name || row.client_name || null,
    order_payable_amount: so?.final_amount ?? so?.grand_total ?? row.order_payable_amount ?? null,
...
export default function TpB2bPaymentAuditTable({ filterParams = {} }) {
    ...
    <>
      ...
      <Dialog open={queryDialog.open} onOpenChange={(open) => !open && !actionLoading && handleCloseQueryDialog()}>
        <DialogContent className="sm:max-w-md">
          ...
          <B2bPaymentVerificationDetails payment={queryDialog.paymentSnapshot} />

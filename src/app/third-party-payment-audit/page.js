...
import {
  ...
  IconRefresh, IconCalendar, IconX, IconClock, IconCircleCheck, IconMessageQuestion,
...
import TpPaymentAuditTable from "./components/TpPaymentAuditTable";
import TpB2bPaymentAuditTable from "./components/TpB2bPaymentAuditTable";

const TP_CHANNEL_KEY = "tp_payment_channel";

function getStoredTpChannel(fallback = "b2c") {
  if (typeof window === "undefined") return fallback;
  const value = sessionStorage.getItem(TP_CHANNEL_KEY);
  return value === "b2b" || value === "b2c" ? value : fallback;
}

function setStoredTpChannel(channel) {
  if (typeof window === "undefined") return;
  if (channel === "b2b" || channel === "b2c") {
    sessionStorage.setItem(TP_CHANNEL_KEY, channel);
  }
}

const B2C_INITIAL_FILTERS = {
  start_date: "", end_date: "", branch_id: "", handled_by: "",
  accountant_id: "", payment_mode_id: "", status: ["pending"],
  order_number: "", receipt_number: "", customer_name: "", amount: "", search: "",
};

const B2B_INITIAL_FILTERS = {
  start_date: "", end_date: "", accountant_id: "", payment_mode_id: "",
  status: ["pending"], order_no: "", receipt_number: "", client_name: "",
  amount: "", search: "",
};
...
const FILTER_LABELS = {
  ...
  handled_by: "Handled By", accountant_id: "Accountant",
  payment_mode_id: "Payment Mode", status: "Status",
  order_number: "Order #", order_no: "Order #",
  customer_name: "Customer", client_name: "Client",
  receipt_number: "Receipt #", amount: "Amount", search: "Search",
};
...
const STATUS_TABS = [
  ...
  { value: "pending", label: "Pending", icon: IconClock, cls: "text-amber-600 border-amber-200 hover:border-amber-400", activeCls: "bg-amber-50 border-amber-400 text-amber-700" },
  ...
  { value: "query_raised", label: "Query Raised", icon: IconMessageQuestion, cls: "text-blue-600 border-blue-200 hover:border-blue-400", activeCls: "bg-blue-50 border-blue-400 text-blue-700" },
];
...
function countActive(f) {
  ...
  return Object.entries(f).filter(([key, v]) => {
    if (key === "status" && Array.isArray(v) && v.length === 1 && v[0] === "pending") return false;
...
export default function ThirdPartyPaymentAuditPage() {
  const [channel, setChannel] = useState(() => getStoredTpChannel("b2c"));
  ...
  const removeChip = (key) => {
    ...
    setFilters(next);
    setAppliedFilters(next);
    if (key === "status") setActiveStatusTab("pending");
        ...
        <div className="mx-auto max-w-[1440px] px-3 py-2 pb-4 space-y-1.5">
          ...
          <div className="flex items-center justify-between gap-2 flex-wrap">
            <div className="flex items-center gap-1.5">
              <div className="bg-violet-500/10 p-1 rounded-lg">
                <IconCurrencyRupee size={14} stroke={2} className="text-violet-600" />
              </div>
              <h1 className="text-sm font-bold tracking-tight text-slate-900">Third-Party Payment Audit</h1>
              <span className="text-[11px] text-slate-400 hidden sm:inline">· External Review of Approved Payments</span>
          ...
          <div className="rounded-xl shadow-sm border border-slate-200 bg-white overflow-visible">
            ...
            {filtersOpen && (
              <div className="border-t border-slate-100 px-2.5 py-2 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-1.5">
                ...
                <AutocompleteField
                  usePortal
                  name="accountant_id" label="Accountant"
                  asyncLoadOptions={(q) => getReferenceOptionsSearch("user.model", { q, limit: 20 })}
                  referenceModel="user.model"
                  getOptionLabel={(o) => o?.name ?? o?.email ?? ""}
                  value={filters.accountant_id ? { id: filters.accountant_id } : null}
                  onChange={(e, v) => fc("accountant_id", v?.id ?? null)} placeholder="Search accountant…"
                />
                <AutocompleteField
                  usePortal
                ...
                />
                <Input
                  name="amount" label="Amount" type="number" value={filters.amount || ""}
                  onChange={(e) => fc("amount", e.target.value || null)} placeholder="Exact amount"
                />
                <Input
          ...
          <div className="rounded-xl border border-slate-200 bg-white overflow-hidden shadow-sm">
            {channel === "b2b" ? (
              <TpB2bPaymentAuditTable key={`b2b-${refreshKey}`} filterParams={appliedFilters} />
            ) : (
              <TpPaymentAuditTable key={`b2c-${refreshKey}`} filterParams={appliedFilters} />
            )}
          </div>
